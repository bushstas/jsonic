<?php
		...
		if ($callback !== null && $callback !== false && $callback !== '') {
			...
			if (!preg_match('/^this\.[a-z]\w*$/i', $callback)) {
				if (!empty($name)) {
					...
					error('Initial параметр <b>'.$initialType.'</b> в классе <b>'.$component['name'].'</b> имеет поле <b>'.$fileld.'</b> с некорректным значением <b>'.$value.'</b><xmp>initial '.$initialType.' = '.$initialValue.'</xmp>Ожидается функция обработчик вида <b>this.onChangeGlobalVar</b><br><br><b>Параметр должен иметь вид:</b> '.getInitialParamExample($initialType));					
				}
			}
		}
	...
	function validateControllersInitials($initials, $initialValue, $initialType, $component) {
		...
		// preg_match_all('/\bon[\'"]*\s*:\s*\{([^\}]+)\}/', $initialValue, $matches);
		// $handlers = $matches[1];
		// if (empty($handlers) && !empty($component['controllers'])) {
		// 	error("В параметре initial <b>controllers</b> класса <b>".$component['name']."</b> должно быть поле on вида <b>{'load': this.onLoad, 'add': this.onAdd}</b>");
		// }
		// foreach ($handlers as $handler) {
		// 	preg_match_all('/[\'"]*\w+[\'"]*\s*:\s*([^,]+)/', $handler, $matches);
		// 	addCallbacksToComponent($component, $matches[1]);
		// }
		...
		// 	if (!preg_match('/^\w+$/', trim($controller))) {
		// 		error('Невалидное название контроллера <b>'.$controller.'</b>');
		// 	}
		// 	if ($classType == 'controller' && $component['name'] == $controller) {
		// 		error('Контроллер <b>'.$controller.'</b> не может содержать сам себя в списке контроллеров');
		// 	}
