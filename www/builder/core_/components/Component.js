...
var onReadyToRender=function(){if(!this.isRendered()){doRendering.call(this);if(this.tempPlaceholder){this.parentElement.removeChild(this.tempPlaceholder);this.tempPlaceholder=null}_G_.get('Core').processPostRenderInitials.call(this)}};
var doRendering=function(){var lvl=_G_.get('Level');this.level=new lvl(this);var content=this.getTemplateMain(this.props,this);if(content)this.level.render(content,this.parentElement,this,this.tempPlaceholder);this.rendered=true;this.onRendered();this.onRenderComplete();this.forEachChild(function(child){if(isFunction(child.onParentRendered))child.onParentRendered.call(child)});delete this.waiting};
...
var unrender=function(){this.elements=null;_G_.get('Core').disposeLinks.call(this);this.disposeInternal();for(var i=0;i<this.inheritedSuperClasses.length-1;i++){this.inheritedSuperClasses[i].prototype.disposeInternal.call(this)}this.level.dispose();this.level=this.listeners=null};
p=c.prototype;
...
p.dispose=function(){_G_.get('State').dispose(this);unrender.call(this);if(this.mouseHandler){this.mouseHandler.dispose();this.mouseHandler=null}this.updaters=null;this.parentElement=null;this.props=null;this.children=null;this.disposed=true;this.initials=null;this.followers=null;this.correctors=null;this.controls=null};
