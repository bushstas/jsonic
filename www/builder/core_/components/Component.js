...
var callFollower=function(propName,propValue){if(__O.has(this.followers,propName))this.followers[propName].call(this,propValue)};
...
p.set=function(propName,propValue){this.props=this.props||{};var props;if(!isUndefined(propValue)){props={};props[propName]=propValue}else if(isObject(propName)){props=propName}else return;var isChanged=false;var changedProps={};var currentValue;for(var k in props){if(__O.has(this.correctors,k))props[k]=this.correctors[k].call(this,props[k],props);currentValue=this.props[k];if(currentValue==props[k])continue;if(isArray(currentValue)&&isArray(props[k])&&__O.equals(currentValue,props[k]))continue;isChanged=true;this.props[k]=props[k];changedProps[k]=props[k]}if(this.isRendered()){if(isChanged)propagatePropertyChange.call(this,changedProps)}changedProps=null};
...
p.removeChild=function(child){if(!child)return;var childId=child;if(isString(child))child=this.getChild(child);else childId=__O.getKey(this.children,child);if(isComponentLike(child))child.dispose();if((isString(childId)||isNumber(childId))&&isObject(this.children)){this.children[childId]=null;delete this.children[childId]}};
...
p.getControl=function(name){return __O.get(this.controls,name)||this.forEachChild(function(child){return child.getControl(name)})};
...
p.forEachControl=function(callback){if(isObject(this.controls))__O.each(this.controls,callback,this)};
p.hasControls=function(){return!__O.empty(this.controls)};
...
p.getChildAt=function(index){return __O.getByIndex(this.children,index)};
...
p.getChild=function(id){return __O.get(this.children,id)};
p.getId=function(){return this.componentId};
p.getElement=function(id){if(isString(id))return __O.get(this.elements,id);else return this.scope||this.parentElement};
