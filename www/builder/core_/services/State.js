...
this.set=function(name,value){var updated,data=name;if(!isUndefined(value)){data={};data[name]=value}var changed={},isChanged=false;for(var k in data){if(vars[k]==data[k])continue;if(isArray(vars[k])&&isArray(data[k])&&__O.equals(vars[k],data[k]))continue;isChanged=true;changed[k]=data[k]}if(isChanged){for(var k in changed){vars[k]=changed[k];var s=subscribers[k];if(isArray(s)){for(var i=0;i<s.length;i++){if(isFunction(s[i][0])){s[i][0].call(s[i][1]||null,changed[k],k)}}}var u=updaters[k];if(isArray(u)){updated=[];for(var i=0;i<u.length;i++){if(updated.indexOf(u[i])==-1){u[i].react(changed);updated.push(u[i])}}}}}updated=changed=data=null};
