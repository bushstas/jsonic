...
var initRoutes=function(rts,parents){parents=parents||[];var tempParents=__O.clone(parents);var name,path;for(var i=0;i<rts.length;i++){name=rts[i]['name'];tempParents.push(name);var children=__O.clone(rts[i]['children']);delete rts[i]['children'];path=rts[i]['path']=tempParents.join('/');initRouteParams(rts[i]);properRoutes[path]=rts[i];if(isArray(children)){initRoutes(children,__O.clone(tempParents))}tempParents=__O.clone(parents)}};
