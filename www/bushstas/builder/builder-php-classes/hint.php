<?php
	...
	switch ($topic) {
		case 'config':
			...
			$items[] = 'Приложение';
			$items[] = false;
			$items[] = "<b>base</b> - Путь к корневой директории проекта, где располагается <b>indexPage</b>
			<xmp>\"base\": \"/user1/\"</xmp>";
			...
			$items[] = "<b>pathToApi</b> - Путь к директории, где располагаются серверные api приложения, относительно корня сайта (относительно <b>indexPage</b>)
			<xmp>\"pathToApi\": \"../../user1-api\"</xmp>";
		...
		case 'user':
			...
			$items[] = 'Если вам не требуется авторизация, просто не указывайте эти пути, тогда приложение перейдет сразу непосредственно к рендерингу';
...
{/if}
		...
		case 'css':
			...
			$items[] = "Файлы стилей могут располагаться где угодно, но обязательно должны иметь расширение <b>.css</b>";
			$items[] = "Файлы стилей могут иметь любые названия, хотя в именовании их по именам классов есть определенный смысл";
			$items[] = "Все файлы, имена которых не соответсвуют ни одному из найденных классов, будут обязательно включены в сборку";
			$items[] = "Но если файл назвать по имени класса, то в случае, когда класс не был использован, и соответсвенно не был включен в сборку, его одноменный файл стилей также будет проигнорирован";
			$items[] = "Таким образом чтобы избежать включения в сборку ненужных стилей, называйте css файлы по именам классов";
			$items[] = "Еще одна польза от такого наименования это использование автодополнения .class селекторов именем файла, таким же образом, что и автодополнение в шаблонах:<xmp>.@ {\n\tdisplay: none;\n}\n\n.@title {\n\tfont-size: 15px;\n}</xmp>";
			$items[] = "Так если файл стилей имеет название LoginForm, то стили будут выглядеть так:<xmp>.login-form {\n\tdisplay: none;\n}\n\n.login-form_title {\n\tfont-size: 15px;\n}</xmp>";
		...
		default:
			$items[] = "Документация  по данному вопросу в разработке";
	}
