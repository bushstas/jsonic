...
p=c.prototype;
p.handleClick=function(){$a=[10,20,30,40,50,60,70]
$i=40};
...
p.getInitials=function(){return{'props':{"a":"Input"}}};
...
p.onRefreshButtonClick=function(){var a=<.@>;$filters.each(function(filter){StoreKeeper.remove('filterStat_'+filter.filterId)});this.refresh()};
p.onFilterClick=function(){};
p.refresh=function(){<:rb>.hide();this.currentFilterIndex=0;this.getCountForFilterWithIndex(0)};
p.onLoaded=function(filters){$filters=>};
...
p.getCountForFilterWithIndex=function(index){var filter=$($filters,index);if(isObject(filter)){__C.get(2).doAction(this,'load',{'filterId':filter.filterId})}else{<:rb>.show()}};
...
p=c.prototype;
p.onResetButtonClick=function(){$reset=true;delay(2500){$reset=false}};
...
p=c.prototype;
p.setControlValue=function(value){$keywords=value['tags']};
p.onChange=function(){==>TenderSearchFormChanged};
p.addRequest=function(){$keywords.addOne([],0)};
p.removeRequest=function(index,isExact){$keywords.removeAt(isExact?index:$keywordsCount-index-1)};
p.onKeywordsChange=function(kw){var kwlen=kw.length,tabs=[],i;for(i=1;i<=kwlen;i++){tabs.push(@request+' '+i)}
$keywordsCount=kwlen,
$tabs=tabs,
$activeTab=kwlen-1;this.appendChild('tabs',kwlen>1);this.forChildren('KeywordsControl',function(child,i){child.set('index',kwlen-i)})};
p.onSelectTab=function(index){index=$keywordsCount-index-1;<:area>.scrollTo(<.@block[index]>,300)};
p.onTagEdit=function(tag){<::editor>.edit(tag);Popuper.skipAll(true)};
p.onTagEdited=function(){Popuper.skipAll(false)};
p.onRemoveRequestClick=function(target){var block=target.getAncestor('.keywords_block');var blocks=<.@block[]>;this.removeRequest(blocks.indexOf(block),true)};
p.getTemplateMain=function(_,$){return[{'p':{'c':'keywords_options'},'t':0,'c':[{'p':{'c':'bold'},'t':1,'c':__[16]},{'p':{'p':{'options':__V[0],'className':'bzk','tooltip':true}},'cmp':'Select','nm':'nonmorph'},{'p':{'c':'bold'},'t':1,'c':__[17]},{'p':{'p':__V[1]},'cmp':'Checkbox','nm':'searchInDocumentation'},{'p':{'p':__V[2]},'cmp':'Checkbox','nm':'registryContracts'},{'p':{'p':__V[3]},'cmp':'Checkbox','nm':'registryProducts'},{'p':{'c':'keywords_add-request'},'t':0,'c':[{'t':1,'c':__[21]},{'p':{'className':'bzj','key':'keywordsNewReq'},'tmp':3}]},{'p':{'c':'tooltip keywords-hint'},'t':0}]},{'p':function(){return{'p':{'items':$.g('tabs'),'activeTab':$.g('activeTab')},'i':'tabs'}},'cmp':'Tabs','e':[22,$.onSelectTab,'remove',$.removeRequest],'n':{'items':'tabs','activeTab':'activeTab'}},{'p':function(){return{'as':'area','c':'keywords_area '+($.g('keywordsCount')>1?'multi':'')}},'t':0,'n':{'c':'keywordsCount'},'c':{'h':function(item,i){return{'p':{'p':{'items':item}},'cmp':'KeywordsControl','e':['edit',$.onTagEdit,14,$.onChange],'nm':'tags'}},'n':'keywords','p':function(){return{'p':$.g('keywords')}}}},{'p':{'i':'editor'},'cmp':'KeywordTagEditor','e':['hide',$.onTagEdited]}]};
...
p=c.prototype;
p.onFocus=function(switched){$switched=>};
p.onRecommendationsChange=function(count){$hasRecomm=count>0};
...
p=c.prototype;
p.onRendered=function(){this.setParams({'registryContracts':1});delay(6000){$:aaa=[9,8,7,6,5,4,3,2,1]
State.dispatchEvent('aaa');delay(3000){$:aaa=[100,200,300,400]
delay(3000){$:aaa=null}}}};
...
p=c.prototype;
p.onLoadFilters=function(filters){$quantity=filters.length};
...
p=c.prototype;
p.getInitials=function(){return{'actions':{"load":{"url":__AC.favorites.get},"add":{"url":__AC.favorites.add},"put":{"url":__AC.favorites.remove}}}};
...
p.getInitials=function(){return{'options':{"key":"filterId","store":false,"storeAs":"filters","storePeriod":"1day","clone":true},'actions':{"load":{"url":__AC.filters.load,"method":"GET","callback":this.onLoad},"save":{"url":__AC.filters.save,"method":"POST","callback":this.onAdd},"set":{"url":__AC.filters.set,"method":"POST"},"subscribe":{"url":__AC.filters.subscribe,"method":"POST","callback":this.onSubscribe}}}};
__G.set(c=__F(),'FiltersStat');
p=c.prototype;
p.getInitials=function(){return{'options':{"key":"filterId","store":false,"storeAs":"filterStat_$filterId","storePeriod":"4hour"},'actions':{"load":{"url":__AC.filterStat.load,"method":"GET"}}}};
__G.set(c=__F(),'RecommendationsLoader');
p=c.prototype;
p.getInitials=function(){return{'actions':{"load":{"url":__AC.keywords.recommendations,"method":"POST","autoset":{"data":"items"}}}}};
__G.set(c=__F(),'Subscription');
p=c.prototype;
p.getInitials=function(){return{'actions':{"load":{"url":__AC.settings.subscr,"method":"GET","autoset":{"options":"opts"}},"save":{"url":__AC.settings.set,"method":"GET"}}}};
__G.set(c=__F(),'UserInfoLoader');
p=c.prototype;
p.getInitials=function(){return{'actions':{"load":{"url":__AC.user.get,"method":"GET"}}}};
__G.set(c=__F(),'Checkbox');
p=c.prototype;
p.onClick=function(){this.toggle('checked');this.dispatchChange()};
p.getControlValue=function(){return $checked?1:0};
p.setControlValue=function(value){$checked=!!value};
...
__G.set(c=__F(),'Input');
p=c.prototype;
p.getControlValue=function(){return<input>.value};
...
p=c.prototype;
p.onRendered=function(){get value;var selected;if(!isUndefined(value)){selected=this.selectByValue(value,true)}
if(!selected){this.selectByIndex(0)}};
p.getChangeEventParams=function(){return{value:$value,title:$title}};
p.selectByValue=function(value,forced){if(!forced&&$value==value)return;get options;if(isArray(options)){for(var i=0;i<options.length;i++){if(options[i]['value']==value){this.selectedIndex=i;if(!forced)$value=value;$title=options[i]['title'];this.syncTooltip(i);return true}}}
return false};
p.selectByIndex=function(index){get options;this.selectedIndex=index;if(isObject(options[index])){if($value==options[index]['value'])return;$value=options[index]['value'],
$title=options[index]['title'];this.syncTooltip(index)}};
p.syncTooltip=function(index){var optionElement=this.getOptionElementAt(index);var tooltipElement=optionElement<.tooltip>};
...
p.onOptionsClick=function(e){var target=e.getTarget('.select_option');if(target&&!target.hasClass('disabled')){var value=target->value;if(this.selectByValue(value)){this.dispatchChange()}
this.hide()}};
p.getOptionElementAt=function(index){return<.@options>.getChildAt(index)};
p.setProperValue=function(value){this.selectByValue(value)};
p.getControlValue=function(){return<input>.value};
p.onClick=function(){$active!;Popuper.watch(this)};
p.hide=function(){$active=false};
p.getTemplateMain=function(_,$){return{'p':function(){return{'c':'select '+(_['className']?_['className']:'')+' '+($.g('active')?'active':''),'sc':1}},'t':0,'n':{'c':'active'},'c':[{'p':{'c':'select_value'},'t':0,'e':[0,$.onClick],'c':[{'v':$.g('title'),'n':'title'},_['tooltip']?{'p':{'className':'bzj','key':$.g('tooltip')},'tmp':3}:'']},{'p':{'c':'select_options'},'t':0,'e':[0,$.onOptionsClick],'c':{'h':function(option){return{'p':{'c':'select_option','_value':option.value},'t':0,'c':[option.title,option.tooltip?{'p':{'className':'bzj','key':option.tooltip},'tmp':3}:'']}},'n':'options','p':function(){return{'p':$.g('options')}}}},{'p':function(){return{'tp':'hidden','n':$.g('name'),'v':$.g('value')}},'t':14,'n':{'n':'name','v':'value'}}]}};
__G.set(c=__F(),'ContainKeywordTags');
p=c.prototype;
p.onRendered=function(){this.resetOptions()};
p.onPickRecommendation=function(value){};
p.onEnter=function(value){__G.get('KeywordTags').prototype.onEnter.call(this,value);var items=$items.join(',').replace(/\#\d/g,'').split(',');<::recommendations>.load(items)};
p.getCorrectedText=function(text){var opt1=$opt1value;var opt2=$opt2value;if(opt1>1||opt2>1){return text+'#'+opt1+'#'+opt2}
return text};
p.resetOptions=function(){$opt1=@noOrder,
$opt2=@withinParagraph;$opt1value=1;$opt2value=1};
...
p=c.prototype;
p.onRendered=function(){this.resetOptions()};
p.getCorrectedText=function(text){get opt1value;if(opt1value>1){return text+'#'+opt1value}
return text};
p.resetOptions=function(){$opt1=@noOrder,
$opt1value=1};
...
p.onOptionClick=function(target){var select=this.getChild('opt'+target->index);if(select){select.show()}};
...
p.tagExists=function(text){var items=$items.join(',').replace(/\#\d/g,'');return items.split(',').has(text)};
...
__G.set(c=__F(),'Tags');
p=c.prototype;
p.onEnter=function(value){value=value.split(',');var a=[],tv;each(value as v){tv=v.trim().toLowerCase();if(!tv.isEmpty()&&!this.tagExists(tv)){a.push(this.getCorrectedText(tv))}}
if(!a.isEmpty()){$items.add(a,0);this.dispatchChange()}};
p.tagExists=function(text){return $items.has(text)};
p.getCorrectedText=function(text){return text};
p.onPickVariant=function(value){this.onEnter(value)};
p.onRemoveButtonClick=function(target){$items.remove(target.prev()->text);this.dispatchChange()};
p.onTagClick=function(target){-->edit(target)};
p.getControlValue=function(){return $items.join(',')};
p.clearControl=function(){$items=[]};
p.onItemsChange=function(items){$count=items.length};
...
p=c.prototype;
p.getControlValue=function(){return<textarea>.value};
...
p=c.prototype;
p.correct=function(params){var tags=[];var maxlen=$(params['containKeyword'],'length',0);maxlen=Math.max(maxlen,$(params['notcontainKeyword'],'length',0));if(maxlen>0){var ck,nck;for(var i=0;i<maxlen;i++){ck=$(params['containKeyword'],i,'').toArray();nck=$(params['notcontainKeyword'],i,'').toArray();tags.push([ck,nck])}}else{tags.push([])}
...
p.getInitials=function(){return{'props':{"action":__AC.orderCall.send,"method":"POST","className":"app-order-call","controls":[__V.nameInput,__V.emailInput,__V.phoneInput,{"caption":__[58],"class":"half-width","controlClass":"Select","controlProps":{"name":"topic","options":Dictionary.orderCallTopics}},{"caption":__[59],"class":"half-width","controlClass":"Select","controlProps":{"name":"date","options":this.getDateOptions()}},{"caption":__[60],"class":"half-width","controlClass":"Select","controlProps":{"name":"time","options":Dictionary.timeOptions}},{"caption":__[61],"controlClass":"Textarea","controlProps":{"name":"comment"}}],"submit":{"value":__[93],"class":"standart-buttongreen-buttonsend-button"}}}};
__G.set(c=__F(),'SupportForm');
p=c.prototype;
p.getInitials=function(){return{'props':{"action":__AC.support.send,"className":"app-order-call","controls":[__V.nameInput,__V.emailInput,__V.phoneInput,{"caption":__[64],"controlClass":"Textarea","controlProps":{"name":"comment"}},{"caption":__[65],"controlClass":"Input","controlProps":{"name":"screenshot","type":"file","accept":"image/*"}}],"submit":{"value":__[66],"class":"standart-buttongreen-buttonsend-button"}}}};
__G.set(c=function(){
Router.addMenu(this);
...
p=c.prototype;
p.onInput=function(value){get options;var len=value.length;var minLength=$(options,'minLength',3);if(isString($(options,'url'))&&len>=minLength){this.delay(this.load,1000,value)}else if(len==0){this.delay();$variants=[]}};
p.load=function(value){Loader.get($($options,'url'),{'token':value},this.onLoad,this)};
p.onLoad=function(data){$currentVariant=null;$variants=data['items']};
p.onFocus=function(){if($variantsCount>0){$active=true}};
p.onChangeVariants=function(variants){var count=isArray(variants)?variants.length:0;$variantsCount=count,
$active=count>0};
p.onBlur=function(){this.delay(function(){$active=false},200)};
p.onEnter=function(value){get currentVariant;if(isNumber(currentVariant)){var e=<.@variant.active>;-->enter(e->value)
this.onEscape();return false}else{this.clear()}};
p.setValue=function(value){<input>.value=value};
p.onEscape=function(){this.clear()};
p.clear=function(){this.delay();<:input>.clear();$variants=[]};
p.onVariantPick=function(target){-->pick(target->value)
this.clear()};
p.onUp=function(){this.highlightVariant(-1)};
p.onDown=function(){this.highlightVariant(1)};
p.highlightVariant=function(step){get variants,currentVariant;if(isArray(variants)&&variants.length>0){var total=variants.length;if(!isNumber(currentVariant)){currentVariant=-1}
currentVariant+=step;if(currentVariant<0){currentVariant=total-1}else if(currentVariant==total){currentVariant=0}
$currentVariant=>}};
p.onChangeCurrentVariant=function(index){var e=<.@variant.active>;@(e).removeClass('active');e=<.@variant[index]>;@(e).addClass('active')};
p.onChangeActive=function(isActive){if(!isActive)$currentVariant=null};
...
p=c.prototype;
p.onAddButtonClick=function(){var value=<input>.value;this.onEnter(value);-->enter(value)};
...
this.redraw()};
p.onMarkedDayClick=function(target){var timestamp=target.innerHTML+'.'+(this.month+1)+'.'+this.year;if(isArray(this.tenderByDates[timestamp])){Dialoger.show(CalendarFavorites,{'title':@calendarFavoritesTitle+' '+Dates.getFormattedDate(timestamp,@calendarFavoritesDateFormat),'tenders':this.tenderByDates[timestamp]})}};
...
p=c.prototype;
p.show=function(){$shown=true;this.reposition();this.onShow()};
p.reposition=function(){var rect=<>.getRect();$marginTop=Math.round(rect.height/-2)+'px',
$marginLeft=Math.round(rect.width/-2)+'px'};
p.hide=function(){$shown=false};
p.close=function(){this.hide();this.onHide()};
p.expand=function(isExpanded){if(isBool(isExpanded)){$expanded=isExpanded}else{$expanded!}};
p.onShow=function(){};
p.onHide=function(){};
...
p=c.prototype;
p.edit=function(element){this.editedElement=element;$text=element.innerHTML,
$shown=true;this.reposition()};
p.reposition=function(text){this.placeTo(document.body);var rect=this.editedElement.getRect();<>.setPosition(rect.left,rect.top)};
p.onChangeText=function(text){var input=<input>;input.value=text;input.focus()};
p.onEnter=function(value){this.editedElement.innerHTML=value;this.hide()};
p.hide=function(){this.close();this.placeBack();-->hide};
p.close=function(){$shown=false};
...
p.send=function(){get action,method;if(action){Loader.doAction(method||'POST',action,this.getControlsData(),this.handleResponse,this)}};
...
p=c.prototype;
p.onRendered=function(){this.button=<>.parentNode;this.addListener(this.button,'click',this.onShowButtonClick)};
p.onClick=function(e){var target=e.getTarget('.popup-menu_button');if(!isNull(target)){get buttons;var idx=target.getData('index');var value=target.getData('value');if(isArray(buttons)&&isObject(buttons[idx])&&isFunction(buttons[idx]['handler'])){buttons[idx]['handler'].call(this,e);return}
this.handleClick(value,target)}};
p.onShowButtonClick=function(){this.onBeforeShow();this.show()};
p.show=function(){var innerElement=<.@inner-container>;var rect=innerElement.getRect();var height=Math.min(rect.height,$(this.options,'maxHeight',400));<>.css({maxHeight:height+'px',height:height+'px'});this.button.addClass('active');Popuper.watch(this)};
p.hide=function(){<>.css({maxHeight:'0',height:'0'});this.button.removeClass('active')};
p.renderButtons=function(items){var*$buttons=[];each(items as item){buttons.push(this.getButtonData(item))}};
...
p=c.prototype;
p.show=function(){$shown=true;Popuper.watch(this)};
p.hide=function(){$shown=false};
...
p.onChangeItems=function(items){var itemsCount=items.length;$itemsCount=itemsCount;-->change(itemsCount)};
...
p=c.prototype;
p.onRendered=function(){get tabs;this.tabWidth=$tabWidth||200;this.tabMargin=$tabMargin||4;if(isArray(tabs)){each(tabs as tab){this.activateTab(idx,!!tab['active'])}}
this.redraw()};
p.redraw=function(){this.hiddenTabs=[];var tabPanelWidth=<>.getWidth();var controlWidth=this.getControlsWidth();var tabs=<.@tab[]>;var totalWidth=0;each(tabs as tab){tab.toggleClass('first',idx==0);if(totalWidth+controlWidth+this.tabWidth+this.tabMargin>tabPanelWidth){tab.hide();this.hiddenTabs[]=idx}else{tab.style.left=totalWidth+'px';totalWidth+=this.tabWidth+this.tabMargin}}
$count=this.hiddenTabs.length};
p.getControlsWidth=function(){var width=0;var restButton=<.tab-rest>;width+=@(restButton).getWidth()+this.tabMargin;var plusButton=<.tab-plus>;width+=@(plusButton).getWidth()+this.tabMargin;return width};
...
p.activateTab=function(tabIndex,isShown){var contents=<>.finds('.'+($containerClass||'tab-content'));console.log(contents)@(contents[tabIndex]).show(isShown);if(isShown){-->select(tabIndex)
this.activeTab=tabIndex}<.content-tab[tabIndex]>.toggleClass('active',isShown)};
...
__G.set(c=__F(),'Tabs');
p=c.prototype;
p.onSelect=function(target){var index=target->index;$activeTab=index;-->select(index)};
p.onRemove=function(target){var index=target.getParent()->index;-->remove(index)};
...
p=c.prototype;
p.correctAndSetText=function(text,changedProps){var corrector=changedProps['corrector'];if(corrector=='list'){var textParts=text.split('|');if(textParts[1]){var temp=[];for(var i=0;i<textParts.length;i++){textPart=textParts[i].split('^');textPart=textPart[1]||textPart[0];if(textPart.charAt(0)==@softSign)textPart=@otherRegions;temp.push(textPart)}
...
p=c.prototype;
p.onLoaded=function(filters){$filters=filters;$total=this.getTotalCount(),
$subscribed=this.getSubscribedCount()};
p.getTotalCount=function(){return Decliner.getCount('filter',$filters)};
p.getSubscribedCount=function(){var subscribedCount=0;$filters.each(function(filter){if(filter['isSubs']==1)subscribedCount++});return Decliner.getCount('subscr',subscribedCount)};
...
p.onResize=function(){var element=<.mainpage-content>;element.setHeight('');var height=element.getHeight();var bodyHeight=document.body.getHeight();if(bodyHeight-100-height>0){element.setHeight(bodyHeight-100)}};
...
p=c.prototype;
p.onLoaded=function(data){if(!User.hasFullAccess()){data['prolongButtonText']=@orderTariff}else if(data['needToProlong']){data['prolongButtonText']=@prolongAccess}
this.set(data)};
...
p.openInformer=function(){var datatable=<::datatable>};
p.openFilter=function(filterId){};
p.onFormExpand=function(){$expanded!};
